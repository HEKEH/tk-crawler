...
import { anchorContacted, assignTask } from '../../services';
...
export default class TaskController {
  ...
  static async anchorContacted(ctx: Context, next: Next) {
    const { org_info, user_info } = ctx.clientInfo!;
    const data = ctx.getRequestData<AnchorContactedRequest>();
    await anchorContacted({
      ...data,
      org_id: org_info.id,
      org_member_id: user_info.id,
    });
  ...
  static async cancelAnchorContact(ctx: Context, next: Next) {
    const { org_info } = ctx.clientInfo!;
    const data = ctx.getRequestData<AnchorContactedRequest>();
    await anchorContacted({
      ...data,
