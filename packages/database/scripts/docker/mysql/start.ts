...
async function main() {
  try {
    ...
    const CONTAINER_NAME = `tk-crawler-mysql-${config.env}`;
    process.env.CONTAINER_NAME = CONTAINER_NAME;

    // 创建 volume
    const dataVolumeName = `${CONTAINER_NAME}-data`;
    await createVolumeIfNotExists(dataVolumeName);

    const logsVolumeName = `${CONTAINER_NAME}-logs`;
    await createVolumeIfNotExists(logsVolumeName);
    ...
    if (await needRebuild(config)) {
      ...
      await execAsync(
        `docker-compose -p ${CONTAINER_NAME} -f docker-compose.yml --env-file ${config.envFile} build`,
      );
      // 记录构建时间
      await writeFile(
    ...
    await execAsync(
      `docker-compose -p ${CONTAINER_NAME} -f docker-compose.yml --env-file ${config.envFile} up -d`,
    );
