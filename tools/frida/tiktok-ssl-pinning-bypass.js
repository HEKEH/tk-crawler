  ...
  Java.perform(() => {
    ...
    PackageManager.getPackageInfo.overload(
      ...
      if (a == name && b == 64) {
        ...
        packageInfo.signatures.value = Java.array(
...
function hook_SSL_CTX_set_custom_verify(library) {
  ...
  try {
    ...
    const SSL_CTX_set_custom_verify = new NativeFunction(f, 'void', [
      'pointer',
      'int',
      'pointer',
    ]);
...
Java.perform(() => {
  try {
    ...
    SSLContextInit.implementation = function (
      keyManager,
      trustManager,
      secureRandom,
    ) {
